...
def crawling_MonsterMart(site_name, href, num, f_num):
    ...
    # 맛
    try:
        flavor = soup.find_all("td", {"class": "label_flavor"})[f_num].text.strip()
        ...
        try:
            ...
            flavor = [
                ...
                if x.strip() != "" and x.strip() != "옵션" and x.strip() != "Flavor"
            ][f_num]
            flavor = "no flavor / have opiton"
    ...
    mingrd = soup.find_all("table", {"id": "label_outer_table"})[f_num]
    ingr_words = None
    if mingrd is None:
        pass
    else:
        mingrd = mingrd.text.strip().replace("\n", "")
        mingrd = remove_part(mingrd).strip()
