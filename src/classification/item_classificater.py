...
def small_classificate(standard_dict, mapping_dict, dataset_list):
    ...
    # 반입 금지 물품 목록
    ban_list = list()
    for item in mapping_dict[944:1029]:
    ...
    for dataset in dataset_list:
        for data in dataset:
            ...
            if bool(list(set(data["원료 성분(단어)"]) & set(ban_list))):  # 반입금지
                ...
                "칩" in data["1회제공량"]
                or "조각" in data["1회제공량"]
                or "chips" in data["1회제공량"]
                or "pieces" in data["1회제공량"]
                or "chip" in data["1회제공량"]
                or "piece" in data["1회제공량"]
            ):  # 과자
            ...
            #     data["소분류"] = "케이크"
                ...
                list(
                    set(data["원료 성분(단어)"]) & set(standard_dict[0]["Unnamed: 1"].split("\n"))
                )
            ):  # 프로틴
                data["대분류"] = "프로틴"
                if float(data["단백질(g)"]) <= float(data["총 탄수화물(g)"]) * 2:
                    data["중분류"] = "식사대용/게이너"
                    data["소분류"] = "식사대용/게이너"
                    result["식사대용/게이너"].append(data["제품명"])
                elif "크레아틴" in data["원료 성분(단어)"]:  # 올인원
                    data["중분류"] = "올인원"
                    data["소분류"] = "올인원"
                    result["올인원"].append(data["제품명"])
                elif (
                    "물" in data["원료 성분(단어)"]
                    or "water" in data["원료 성분(단어)"]
                    or "정제수" in data["원료 성분(단어)"]
                ):  # RTD/드링크
                    data["중분류"] = "RTD/드링크"
                    data["소분류"] = "RTD/드링크"
                    ...
                    list(
                        set(data["원료 성분(단어)"]) & set(standard_dict[0]["전체 원료"].split("\n"))
                    )
                ) and bool(
                    list(
                        set(data["원료 성분(단어)"])
                    ...
                    if bool(
                        ...
                        if bool(
                            list(
                                set(data["원료 성분(단어)"])
                                & set(standard_dict[6]["전체 원료"].split("\n"))
                            )
                        ):
                            data["소분류"] = "유청 + 기타"
                    ...
                    if (
                        ...
                        data["소분류"] = "농축유청단백(WPC)"
                        result["농축유청단백(WPC)"].append(data["제품명"])
                    elif bool(
                        list(
                            set(data["원료 성분(단어)"])
                        ...
                        data["소분류"] = "분리유청단백(WPI)"
                        result["분리유청단백(WPI)"].append(data["제품명"])
                    elif bool(
                        list(
                            set(data["원료 성분(단어)"])
                        ...
                        result["가수분해농축유청단백(WPH)"].append(data["제품명"])
                    elif bool(
                        list(
                            set(data["원료 성분(단어)"])
                        ...
                        result["가수분해분리유청단백(WPIH)"].append(data["제품명"])
                    else:
                        data["소분류"] = "유청단백기타"
                    ...
                    list(
                        set(data["원료 성분(단어)"]) & set(standard_dict[8]["전체 원료"].split("\n"))
                    )
                ):  # 우유단백
                    data["중분류"] = "우유단백"
                    if (
                        ...
                        data["소분류"] = "농축우유단백(MPC)"
                        result["농축우유단백(MPC)"].append(data["제품명"])
                    elif bool(
                        list(
                            set(data["원료 성분(단어)"])
                        ...
                        data["소분류"] = "분리우유단백(MPI)"
                        result["분리우유단백(MPI)"].append(data["제품명"])
                    elif bool(
                        list(
                            set(data["원료 성분(단어)"])
                    ...
                    list(
                        set(data["원료 성분(단어)"]) & set(standard_dict[12]["전체 원료"].split("\n"))
                    )
                ):  # 식물성단백
                    data["중분류"] = "식물성단백"
                    if (
                        ...
                        data["소분류"] = "혼합혼합식물성단백"
                        result["혼합식물성단백"].append(data["제품명"])
                    elif bool(
                        list(
                            set(data["원료 성분(단어)"])
                ...
                if bool(
                    list(
                        set(data["원료 성분(단어)"]) & set(standard_dict[19]["전체 원료"].split("\n"))
                    )
                ):  # 아미노산
                    data["중분류"] = "아미노산"
                    ...
                    list(
                        set(data["원료 성분(단어)"]) & set(standard_dict[25]["전체 원료"].split("\n"))
                    )
                ):  # 부스터
                    data["중분류"] = "부스터"
                    if (
                ...
                list(set(data["원료 성분(단어)"]) & set(standard_dict[34]["전체 원료"].split("\n")))
            ):  # 다이어트보조제
                data["대분류"] = "다이어트보조제"
                data["중분류"] = "다이어트보조제"
                if (
                    ...
                    list(
                        set(data["원료 성분(단어)"]) & set(standard_dict[34]["분류기준"].split("\n"))
                    )
                ):  # 녹차추출물
                    data["소분류"] = "녹차추출물"
                    ...
                    list(
                        set(data["원료 성분(단어)"]) & set(standard_dict[35]["분류기준"].split("\n"))
                    )
                ):  # 공액리놀레산
                    data["소분류"] = "공액리놀레산"
                    ...
                    list(
                        set(data["원료 성분(단어)"]) & set(standard_dict[36]["분류기준"].split("\n"))
                    )
                ):  # 가르시니아
                    data["소분류"] = "가르시니아"
                    ...
                    list(
                        set(data["원료 성분(단어)"]) & set(standard_dict[37]["분류기준"].split("\n"))
                    )
                ):  # 키토산
                    data["소분류"] = "키토산"
                    ...
                    list(
                        set(data["원료 성분(단어)"]) & set(standard_dict[38]["분류기준"].split("\n"))
                    )
                ):  # 카르니틴
                    data["소분류"] = "카르니틴"
                    ...
                    list(
                        set(data["원료 성분(단어)"]) & set(standard_dict[39]["분류기준"].split("\n"))
                    )
                ):  # 콜레우스포스콜리
                    data["소분류"] = "콜레우스포스콜리"
                    ...
                    list(
                        set(data["원료 성분(단어)"]) & set(standard_dict[40]["분류기준"].split("\n"))
                    )
                ):  # 그린커피추출물
                    data["소분류"] = "그린커피추출물"
