...
def crawling_Ople(site_name, href, num):
    ...
    try:
        ...
        if ingrdList:
            ingrd = ingrdList[-1]
            ingrd = remove_part(ingrd).strip()
            ingr_words = substract_ingredient(ingrd, ingr_list)
    ...
    for box in soup.find_all("div", {"class": "box_item_title"}):
        if box.find("img") is not None:
            detail_images = box.find("img").get("src")
