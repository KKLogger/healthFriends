...
def load_url(opt, site_name):
    ...
    """
    if opt == 1:
        with open(f"data/urls_{site_name}.pickle", "rb") as f:
            urls = pickle.load(f)
    elif opt == 2:
...
if __name__ == "__main__":
    start = time.time()
    # site_names = ["365Muscle", "iHerb", "IronMax", "Kingmass", "MonsterMart"]
    site_names = ["Ople"]
    # site_names=['365Muscle','iHerb','IronMax','Kingmass','MonsterMart','MyProtein','Ople']
    createFolder("data")
    createFolder("image")
    for site_name in site_names:
        createFolder(f"image/{site_name}")
        urls = load_url(1, site_name)
        max_url = len(urls)
        # max_url =3
        (
            image_list,
        ...
        df["기타성분"] = etc_list
        df.to_excel(f"data/{site_name}.xlsx")
        print("total time : ", time.time() - start)
        with open(f"data/result_{site_name}.json", "w", encoding="euc-kr") as f:
